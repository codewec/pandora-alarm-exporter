...
func TestCoordinatesNullAndFullRefresh(t *testing.T) {
	full := false
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/users/login":
		...
		default:
			fmt.Fprint(w, `{"ts":1,"stats":{"a":{"x":55,"voltage":null,"fuel":"NaN"}}}`)
		}
	}))
	defer s.Close()
	c, _ := NewClient(s.URL, "u", "p", time.Second)
	e := NewExporter(c, true, time.Minute)
	if err := e.Poll(context.Background()); err != nil {
		t.Fatal(err)
	}
	w := httptest.NewRecorder()
	e.ServeHTTP(w, httptest.NewRequest("GET", "/metrics", nil))
	if !strings.Contains(w.Body.String(), `pandora_latitude_degrees{device_id="a"} 55`) || strings.Contains(w.Body.String(), "pandora_fuel_ratio") {
		t.Fatal(w.Body.String())
	}
	full = true
	e.full = time.Now().Add(-time.Hour)
	if err := e.Poll(context.Background()); err != nil {
		t.Fatal(err)
	}
	w = httptest.NewRecorder()
	e.ServeHTTP(w, httptest.NewRequest("GET", "/metrics", nil))
	if strings.Contains(w.Body.String(), "device_id=") {
		t.Fatal("deleted device remains")
	}
}
