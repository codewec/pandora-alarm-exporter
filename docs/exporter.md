...
HTTP-опрос настраивается в .env: PANDORA_POLL_INTERVAL=5m. Поддерживаются длительности с единицами, например 1m или 300s; минимум 10s. Приоритет: --poll-interval → переменная окружения / .env → 5m. Изменение .env требует перезапуска экспортера.
